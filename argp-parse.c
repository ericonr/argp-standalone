                ...
                /* A real option (not just documentation).  */
                {
                    ...
                    /* OPT can be used as a long option.  */
                    {
                        ...
                        cvt->long_end->has_arg
                            = (real->arg ? (real->flags & OPTION_ARG_OPTIONAL ? optional_argument
                                                                              : required_argument)
                                         : no_argument);
                        cvt->long_end->flag = 0;
...
parser_parse_opt(struct parser* parser, int opt, char* val)
{
       ...
       determine which group OPT came from.  */
    {
        ...
        err = group_parse(&parser->groups[group_key - 1],
                          &parser->state,
       ...
       with each option.  */
    {
        ...
        if (group_key == 0)
            __argp_error(
                &parser->state, "-%c: %s", opt, dgettext(parser->argp->argp_domain, bad_key_err));
        else
        {
            struct option* long_opt = parser->long_opts;
            while (long_opt->val != opt && long_opt->name)
                long_opt++;
            __argp_error(&parser->state,
                         "--%s: %s",
    ...
    /* Give getopt a chance to parse this.  */
    {
        ...
        if (parser->state.flags & ARGP_LONG_ONLY)
            opt = getopt_long_only(
                parser->state.argc, parser->state.argv, parser->short_opts, parser->long_opts, 0);
        else
            opt = getopt_long(
                parser->state.argc, parser->state.argv, parser->short_opts, parser->long_opts, 0);
